...
var formatStr = '% 5s | % 12s | % 7s | % 10s | % 10s | % 12s | % 12s | % -20s | % -25s ';
...
var log2Term = function(re){
    ...
    var p = printf(formatStr, 'Trade', hd.tradeDate, hd.symbol, hd.amount, hd.unitPrice,
                   hd.totalValue, hd.sharesOwned, hd.insider, hd.relationship);
    console.info(p.underline.tableHd);

    re.data.forEach(function(dt){
        p = printf(formatStr, dt.tradeType, dt.tradeDate,
                   dt.symbol, dt.amount, dt.unitPrice, dt.totalValue, dt.sharesOwned,
                   dt.insider, dt.relationship);
        console.info(p);
    });
