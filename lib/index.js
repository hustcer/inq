...
var dataFormat = '% 3s | % 12s | % 6s | % 10s | % 10s | % 15s | % 12s | % -22s | % -25s ';
...
log2Term = function(re){
    ...
    var p = prt(dataFormat, hd.tradeType, hd.tradeDate, hd.symbol, hd.amount, hd.unitPrice,
                hd.totalValue, hd.sharesOwned, hd.insider, hd.relationship);

    console.info(p.underline.header);

    re.data.forEach(function(dt){
        p = prt(dataFormat, dt.tradeType, dt.tradeDate, dt.symbol, dt.amount, dt.unitPrice,
                dt.totalValue, dt.sharesOwned, dt.insider, dt.relationship);
